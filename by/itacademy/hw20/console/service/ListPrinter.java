        ...
        if (list.isEmpty()) {
            ...
            for (Object object : list) {
                if (object instanceof Car) {
                    System.out.printf("%d. %s %s %s\n", list.indexOf((Car) object) + 1, ((Car) object).getNumber(),
                            ((Car) object).getProducer(), ((Car) object).getType());
                } else if (object instanceof CarMaster) {
                    System.out.printf("%d. %s %s\n", list.indexOf((CarMaster) object) + 1,
                            ((CarMaster) object).getName(), ((CarMaster) object).getMasterType());
                    ...
                    System.out.printf("%d. %s. Order status - %s. Duration time - %s days. Car - %s %s %s. " +
                            ...
                            ((Order) object).getOrderNumber(), ((Order) object).getOrderStatus(),
                            ((Order) object).getOrderTime(), ((Order) object).getOrderCar().getNumber(),
                            ((Order) object).getOrderCar().getProducer(), ((Order) object).getOrderCar().getType(),
